...
ROS 2（Robot Operating System 2）是机器人领域的标准软件中间件。它提供了节点间通信、硬件抽象、包管理等功能，是后续所有机器人模块（导航、SLAM、机械臂控制）的基础。
本节将在 reComputer Robotics J5011 上安装 ROS2 Humble，编写发布者/订阅者节点，并实现跨机通信。本章以"安装 ROS2 → 运行第一个 Demo → 搭建工作空间"为主线，完成 reComputer Robotics J501 与 Host PC 双端的 ROS 2 开发环境准备。

## 1.4.1 ROS2 架构概述

本节分别介绍 ROS2 与 DDS 是什么、如何协同工作，详细背景参考[官方文档](https://docs.ros.org/en/humble/index.html)。
...
![ROS 2 + DDS 通信架构示意图](images/1.4.1_dds_architecture.png)
...
ROS2 Humble 在 reComputer 上未预装，需要手动安装。本节提供两种安装方式。

### (1) 方式一：鱼香肉丝一键安装（推荐）

鱼香肉丝（fishros）是 ROS 一键安装工具，自动配置国内镜像源，适合国内网络环境。
...
![Host PC 安装 ROS 2 成功](images/1.4.2_hostpc_ros2_installed.png)
...
`colcon` 是 ROS2 的官方构建系统，用于编译 workspace 中的多个包。

### (1) 创建 workspace

```bash
# [reComputer 本机终端] 创建 ROS2 workspace
mkdir -p ~/ros2_ws/src
cd ~/ros2_ws
...
```bash
# [reComputer 本机终端] source workspace 环境变量
...
本节编写一个**机器人指挥官 Demo**：reComputer 作为机器人端，定时发布系统状态（CPU 温度、内存、磁盘），同时接收 Host PC 发来的控制指令（点亮/熄灭状态灯、发送问候语）。这个 Demo 模拟了真实机器人中"状态上报 + 指令下发"的双向通信模式。

### (1) 创建 Python 包

```bash
# [reComputer 本机终端] 创建 ROS2 Python 包
cd ~/ros2_ws/src
...
![创建 ROS 2 Python 包](images/1.4.4_create_pkg.png)
...
这个节点运行在 Host PC 上，订阅 reComputer 的状态，同时允许用户通过命令行发送控制指令。代码也在 J501 上创建，后续通过 `scp` 同步到 Host PC。

```bash
# [reComputer 本机终端] 创建指令控制台节点（后续同步到 Host PC）
...
```bash
# [reComputer 本机终端 - 终端 1] 运行机器人状态节点
source ~/ros2_ws/install/setup.bash
...
```bash
# [reComputer 本机终端 - 终端 2] 发送问候语（与控制台 CMD_MAP 中的指令 3 一致）
...
## 1.4.5 跨机通信 Demo：远程指挥 reComputer 机器人

ROS2 的分布式通信基于 DDS，不同主机上的节点通过多播自动发现并建立点对点通信（两机之间没有路由器时，可通过 `ROS_DOMAIN_ID` 手动配置）。

### (1) 网络拓扑

![ROS 2 跨机通信拓扑图](images/1.4.5_network_topology.png)

### (2) 同步代码到 Host PC

跨机运行需要 Host PC 上也有相同的包。通过 `scp` 从 reComputer 同步：

```bash
# [Host PC 终端] 从 J501 同步 j501_robot 包
mkdir -p ~/ros2_ws/src
scp -r <reComputer_user>@<reComputer_ip>:~/ros2_ws/src/j501_robot ~/ros2_ws/src/
...
![Host PC 指令控制台启动](images/1.4.5_hostpc_console.png)
...
> **确认**：Host PC 成功订阅 reComputer 的状态上报，同时 reComputer 成功接收 Host PC 的控制指令。双向跨机通信完全正常！

### (4) 查看远程话题

```bash
# [Host PC 终端] 查看 reComputer 上发布的话题
ros2 topic list
```
...
```bash
# [reComputer 本机终端] 设置 DOMAIN_ID 为 42
...
## 1.4.6 Isaac ROS 简介与 image_pipeline 验证
...
| 项目                       | 要求                        | reComputer Robotics J5011 实机状态          |
| ------------------------ | ------------------------- | ------------------ |
...
```bash
# [reComputer 终端] 验证 Isaac ROS 前置条件
echo "=== JetPack ==="
cat /etc/nv_tegra_release 2>/dev/null | head -1; echo "=== Docker ==="
docker --version
echo "=== ROS2 ==="
source /opt/ros/humble/setup.bash && echo "ROS_DISTRO=$ROS_DISTRO"
echo "=== VPI ==="; dpkg -l | grep -i "nvidia-vpi " | awk "{print \$2, \$3}"
echo "=== 磁盘 ==="; df -h / | tail -1'
```
...
**步骤 1：克隆 isaac_ros_common 仓库**

```bash
# [reComputer 本机终端] 克隆 Isaac ROS 公共工具仓库
...
```bash
# [reComputer 本机终端] 启动开发容器（首次会自动拉取镜像，约 15-20 分钟）
...
> **后续课程衔接**：
>
> - **M2.2** 将使用 `isaac_ros_image_pipeline` 实现相机图像的 GPU 加速去畸变和格式转换
> - **M2.4** 将使用 `isaac_ros_apriltag` 实现视觉标签定位
...
- **1.2** 系统刷机与配置 — JetPack 6.2 刷写、性能模式切换
- **1.3** 配置开发环境 — Jtop、Docker、uv、Pytorch等等
- **1.4** ROS 2 Humble — 安装、colcon workspace、节点编写、跨机通信

**下一步**：M2 感知层 — 从"看见"开始，接入 GMSL2 多相机与深度相机，利用 GPU 加速图像处理。

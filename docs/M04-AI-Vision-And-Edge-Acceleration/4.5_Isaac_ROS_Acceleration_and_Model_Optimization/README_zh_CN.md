...
单模型跑得动，不等于四个模型能一起跑。4.1–4.4 各自验收时，机器上只有一个 Engine 在吃 GPU、一份显存、一条数据流；把它们串成「检测 → 分割 → 跟踪」的感知管线后，CPU 与 GPU 之间的拷贝次数、Engine 之间的显存争抢、每帧的 kernel 启动开销会同时出现，整条管线的延迟不再等于各模型延迟之和。

解决办法是把四个模型搬进同一条 Isaac ROS 管线：先用 NITROS 消掉多余的 GPU 内存拷贝，再按「固定输入形状 → INT8 量化 → CUDA Graph → DLA 卸载 → 系统层」的顺序逐层优化，最后用一套固定口径把优化前后的延迟、吞吐、显存、功耗与温度测成可复现的表格。做完这一页，你手上会有一份能在 J501 上反复运行的性能基准，而不是一组只存在于聊天记录里的 FPS 数字。
...
| 跑通 | 四页模型的 Engine 怎样装进同一个 ComposableNodeContainer 而互不挡路 | 在 J501 上让检测、分割、跟踪三路话题同时输出 |
| 测准 | trtexec、nsys、jtop、话题探针各自回答什么问题、各自的盲区在哪 | 交出一份优化前后六项指标齐全的对照表 |

整条管线可以拆成四段：相机输入与预处理 → NITROS 传输 → 多模型推理 → 后处理与发布。本页的推进方式是四段一起跑起来、先测出基线，再一段一段改；每改一段，都用同一份视频、同一预热帧数重测一次，否则你无法判断是优化起了作用，还是环境刚好凉快了一点。
...
| ![硬件与软件清单](./images/ILxbbVYDhoQJk6x2hyvc1CaWnLf.png) | ![硬件与软件清单](./images/KdgqbOCGqoZ2I1xwFScchfuAnWh.png) | ![硬件与软件清单](./images/TASRbYXOjoZVPix2gw3cKpgKn6b.png) |
| --- | --- | --- |
| reCImputer mini J501  + GMSL拓展板 |   | GMSL摄像头 / USB 摄像头 |

## 前置基础

- **M4.1–M4.4**：YOLO 检测、多目标跟踪、语义分割、6D 姿态估计四页各自完成过独立部署，本页迁移的正是这四页的 Engine、后处理代码与话题定义。这四页没跑通，本页就没有可迁移的对象。
...
要交出的是同一个进程内的 ComposableNodeContainer：相机解码与预处理只做一次，检测与分割并列推理，检测结果再进跟踪节点。下面这张表就是本页的话题契约，订阅端按它写，不要凭印象猜消息类型。
...
| `/m4/tracks` | `m4_tracking/TrackedObjects`（4.2 的自研消息，带 track ID、速度与跟踪器状态，随工作空间一起安装）。**不要**用 `vision_msgs/Detection2DArray` 代替：它没有速度字段，也没有跟踪器状态，正是 4.2 否决的方案 | 跟踪节点 / [M6 决策层：定位导航与路径规划](https://seeedstudio.feishu.cn/docx/LTEkdYEgEo6xqwx0IfYcsfrSnYf) 与 [M8 执行层：云台与主动视觉](https://seeedstudio.feishu.cn/docx/FjJCdzbHFolniOxenGocRGMOnNP) |
| `/m4/pose` | `vision_msgs/Detection3DArray`（Isaac ROS 姿态节点的输出类型，TF 帧默认 `fp_object`）。4.4 的包装节点会把它折成自研 `ObjectPoseArray` 供抓取端使用，那就是额外一层转换，本页不隐式依赖它 | 4.4 的姿态节点 / M9 |

启动统一管线，然后在另外三个终端分别看三路话题的频率
...
- 跟踪节点是 CPU 侧代码（来自 4.2 的 ByteTrack / Bot-SORT），它消费的是检测框而不是图像；放在后处理之后，避免它阻塞 GPU 侧回调。

- 先量出「未优化的多模型管线」读数：三路话题同时 ≥30 Hz 是目标，达不到就把实际数字记下来，下一步逐项改。这个 30 Hz 只对检测 + 分割 + 跟踪成立。

- `/m4/tracks` 是 4.2 的 `/tracking/objects` 经 `remap` 改名而来，类型仍是 4.2 的自研 `m4_tracking/TrackedObjects`；重映射写在 launch 里，别让消费端去猜主题名。
...
- 类型按上面的契约表写：4.4 姿态节点的输出是 `vision_msgs/Detection3DArray`、TF 帧默认 `fp_object`，不是 `geometry_msgs/PoseStamped`；照 PoseStamped 写订阅端会收不到数据。
